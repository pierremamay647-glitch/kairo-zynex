...
export async function startSession(targetNumber, handler, makePrimary = true, onPairingCode = () => {}, isReconnect = false, onConnected = () => {}) {
    ...
    // Only request a NEW pairing code on the first call for this number.
    // Baileys normally closes/reopens the socket once while waiting for the
    // user to enter the code on their phone — re-requesting a code on that
    // internal reconnect would silently issue a second, different code and
    // break the one already shown on the pairing website.
