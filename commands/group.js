        ...
        if(messageBody.toLowerCase().includes("on")){
            ...
            await client.sendMessage(remoteJid, {text:"*𝙰𝙽𝚃𝙸𝙻𝙸𝙽𝙺 𝙳𝙸𝚂𝙰𝙱𝙻𝙴*"})
              ...
              if (configManager.config && configManager.config.users[number]) {
                ...
                configManager.config.users[number].antilink = "kick"
            }


            configManager.save()

            await client.sendMessage(remoteJid, {text:"*𝙰𝙽𝚃𝙸𝙻𝙸𝙽𝙺 𝙺𝙸𝙲𝙺 𝙼𝙾𝙳𝙴 𝙴𝙽𝙰𝙱𝙻𝙴*"})
        }
...
export async function welcome(update, client) {
    ...
    // Per-group toggle (ON by default). Configured with .welcome on/off in that group.
    const state = configManager.config?.groups?.[groupId]?.welcome !== false;
...
*💂 POWERED BY KAIRO ZYNEX*
                ...
                await client.sendMessage(update.id, {
                    image: pp ? { url: pp } : fs.readFileSync(path.join(process.cwd(), 'menu.jpg')),
                    caption: welcomeMsg,
...
// ─── .welcome on/off ────────────────────────────────────────────────────────
// Per-group toggle: only that group's admins (or the bot owner) can change it.
export async function welcomeToggle(message, client, args, isOwner) {
    ...
    if (!isOwner && !senderIsAdmin) {
        return client.sendMessage(remoteJid, { text: "❌ *Only this group's admins or the bot owner can configure this.*" }, { quoted: message });
    }

    configManager.config.groups = configManager.config.groups || {};
    configManager.config.groups[remoteJid] = configManager.config.groups[remoteJid] || {};

    const option = (args[0] || '').toLowerCase();

    if (option === 'on') {
        configManager.config.groups[remoteJid].welcome = true;
        configManager.save();
        return client.sendMessage(remoteJid, { text: '✅ *Welcome & goodbye messages turned ON* for this group.' }, { quoted: message });
    }

    if (option === 'off') {
        configManager.config.groups[remoteJid].welcome = false;
        configManager.save();
        return client.sendMessage(remoteJid, { text: '✅ *Welcome & goodbye messages turned OFF* for this group.' }, { quoted: message });
    }

    const current = configManager.config.groups[remoteJid].welcome !== false;
...
export default { kick, kickall, promote, demote, bye, pall, dall, mute, unmute, gclink, antilink, linkDetection, purge, welcome, welcomeToggle, gcid, mentiondetect, isAdmin };
